...
def test_debug_threshold_logs_warning_for_out_of_range(caplog: pytest.LogCaptureFixture) -> None:
    with caplog.at_level("DEBUG"):
...
def test_load_model_uses_cache(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    model = DummyModel()
    mls._MODEL = model
...
def test_load_model_file_not_found(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_load_model_joblib_exception(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    model_path = tmp_path / "model.joblib"
    model_path.write_text("dummy", encoding="utf-8")
...
def test_load_threshold_success(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_load_threshold_uses_cache(monkeypatch: pytest.MonkeyPatch) -> None:
    mls._THRESHOLD = 0.77
